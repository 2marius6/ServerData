    ...
    if($_SESSION['role']==='admin') {
        ...
        if (mysqli_num_rows($result1) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result1)) {
                $email = $row['email'];
                $sql = "SELECT AVG(cpu_temp), AVG(cpu_load), AVG(ram_load), AVG(storage_load), user FROM srv_data_t WHERE user='$email' AND (time_stamp BETWEEN '$dateMinusMonth' AND '$date')";
