<?php
    ...
    $dateMinusDay = date('Y-m-d H:i:s', time() - 86400);
    $i=0;
    if($_SESSION['role']==='admin') {
        $sql1 = "SELECT * from login WHERE role='user'";
        $result1 = mysqli_query($conn,$sql1);
        if (mysqli_num_rows($result1) > 0) {
            echo "<table class='table table-striped table-sm js-sort-table' id='table-history'>";
            echo "<thead>";
            echo "<tr>";
            ...
            while ($row = mysqli_fetch_assoc($result1)) {
                $email = $row['email'];
                $sql = "SELECT AVG(cpu_temp), AVG(cpu_load), AVG(ram_load), AVG(storage_load), user FROM srv_data_t WHERE user='$email' AND (time_stamp BETWEEN '$dateMinusDay' AND '$date')";
        ...
        echo "</script>";
        while($i<24){
            $sql="SELECT AVG(cpu_temp), AVG(cpu_load), AVG(ram_load), AVG(storage_load) FROM srv_data_t WHERE user='$email' AND (time_stamp BETWEEN '$dateMinusHour' AND '$date') ORDER BY id DESC";
            $result = mysqli_query($conn,$sql);
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    $time=json_encode(substr(($date),-8,2) . ":00");
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    echo "<tr>";
                    echo "<td>" . substr($date,0,13) . ":00" . "</td>";
                    echo "<td>" . round($row['AVG(cpu_temp)'],2) . "</td>";
                    echo "<td>" . round($row['AVG(cpu_load)'],2) . "</td>";
                    echo "<td>" . round($row['AVG(ram_load)'],2) . "</td>";
                    echo "<td>" . round($row['AVG(storage_load)'],2) . "</td>";
            ...
            $i++;
            $date=$dateMinusHour;
            $dateMinusHour=date('Y-m-d H:i:s',(strtotime ( '-1 hour' , strtotime ($dateMinusHour) ) ));;
        }
        echo "<script type='text/javascript'>";
