<?php
    ...
    $dateMinusWeek = date('Y-m-d H:i:s', time() - 604800);
    $i=0;
    if($_SESSION['role']==='admin') {
        ...
        if (mysqli_num_rows($result1) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result1)) {
                $email = $row['email'];
                $sql = "SELECT AVG(cpu_temp), AVG(cpu_load), AVG(ram_load), AVG(storage_load), user FROM srv_data_t WHERE user='$email' AND (time_stamp BETWEEN '$dateMinusWeek' AND '$date')";
        ...
        echo "</script>";
        while($i<7){
            $sql="SELECT AVG(cpu_temp), AVG(cpu_load), AVG(ram_load), AVG(storage_load) FROM srv_data_t WHERE user='$email' AND (time_stamp BETWEEN '$dateMinusDay' AND '$date') ORDER BY id DESC";
            $result = mysqli_query($conn,$sql);
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    $time=json_encode(substr($date,0,10));
            ...
            $i++;
            $date=$dateMinusDay;
            $dateMinusDay=date('Y-m-d H:i:s',(strtotime ( '-1 day' , strtotime ($dateMinusDay) ) ));;
        }
        echo "<script type='text/javascript'>";
